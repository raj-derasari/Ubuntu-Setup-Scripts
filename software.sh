...
if [[ -z $PRINTMODE ]]; then
	if [ $Install_Docker -eq 1 ]; then
		...
		$addaptrepo "deb $_architecture https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable"
	fi
...
if [ $Install_PulseAudioEqualizer -eq 1 ]; then
	...
	$apt_prefix pusleaudio-equalizer
