...
sudo apt-get install -y --install-recommends build-essential cmake curl pkg-config libx264-dev libgtk2.0-dev
sudo apt-get install -y --install-recommends gcc-6-base:i386 libbz2-1.0:i386 libc6:i386 libgcc1:i386 libgpm2:i386 libncurses5:i386 libstdc++6:i386 libc6-i386 lib32z1 lib32ncurses5 lib32z1 libtinfo5:i386
sudo apt-get install -y --install-recommends libbz2-dev libssl-dev libreadline-dev libsqlite3-dev tk-dev
sudo apt-get install -y --install-recommends gconf2 gconf2-common gconf-service gconf-editor gconf-defaults-service gconf-service-backend
sudo apt-get purge -y --auto-remove gnome-disk-utility gnome-mplayer simple-scan xfburn
sudo apt-get purge -y --auto-remove abiword* alsa* audacious* gnome-mpv gnumeric* leafpad pidgin* slypheed* transmission*
sudo apt-get purge -y --auto-remove adwaita-icon-theme dmz-cursor-theme gnome-icon-theme gtk-update-icon-cache humanity-icon-theme
...
sed -i 's/amixer -q sset Master toggle/amixer -D pulse set Master toggle/g' /home/thanos/.config/openbox/lubuntu-rc.xml
...
load-module module-equalizer-sink | sudo tee -a /etc/pulse/default.pa
...
sudo apt-get install -y -f
virtualenv --system-site-packages -p python3 /etc/skel/.virtualenvs/venv1
source /etc/skel/.virtualenvs/venv1/bin/activate
sudo apt-get update
